...
function App() {
  ...
  const [isSyncScroll, setIsSyncScroll] = useState(true); // 新增的状态
  ...
  const handleFileChange = (e) => {
    console.log('File change event:', e); // 输出整个事件对象
    ...
    <div className="page">
      <div className="bar">
        <Suspense fallback={<div>Loading...</div>}>
          <Navbar onFileChange={(e) => handleFileChange(e)} onReset={() => setIsResetVisible(true)} isSyncScroll={isSyncScroll} setIsSyncScroll={setIsSyncScroll} />
          <Toolbar setMarkdownContent={setMarkdownContent} />
        </Suspense>
      </div>
      <div className="container">
        <Suspense fallback={<div>Loading...</div>}>
          <Editor markdownContent={markdownContent} setMarkdownContent={setMarkdownContent} setIsSyncScroll={setIsSyncScroll} />
          <Preview markdownContent={markdownContent} setIsSyncScroll={setIsSyncScroll} />
        </Suspense>
      </div>
      <Suspense fallback={<div>Loading...</div>}>
        <Modal
